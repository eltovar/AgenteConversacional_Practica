...
def process_message(session_id: str, user_message: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 2. PRE-ROUTER: DETECCIÓN DE SESIÓN NUEVA
        if state.status == ConversationStatus.RECEPTION_START and len(state.history) == 0:
            logger.info("[ORCHESTRATOR] Sesión nueva detectada - Enviando mensaje de bienvenida")
