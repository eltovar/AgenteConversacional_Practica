...
Uso:
    python scripts/diagnostico/simulate_reminder.py              # simula recordatorios + followups
    python scripts/diagnostico/simulate_reminder.py reminders    # solo recordatorios
    python scripts/diagnostico/simulate_reminder.py followups    # solo seguimientos
"""
import asyncio
import os
import sys
...
if stdout_encoding is not None:
    encoding_name = str(stdout_encoding)
...
async def main():
    ...
    if mode not in ("all", "reminders", "followups"):
        print(f"Uso: python scripts/diagnostico/simulate_reminder.py [all|reminders|followups]")
        sys.exit(1)
